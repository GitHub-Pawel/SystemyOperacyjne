/* Ponizszy modul zmodyfikowalem w ten sposob, 
 * aby kazda nowo utworzona semafora miala ustawiony
 * stan poczatkowy na opuszczony. Dodatkowo wylaczylem
 * wyswietlanie na stdout informacji o utworzeniu lub
 * dolaczeniu sie do zbioru semaforow. */
...
/* Wszystkie semafory maja ustawiany stan poczatkowy na opuszczony.           */
/******************************************************************************/
	...
	if ((semid = semget(key, liczba_elementow, 0644 )) < 0) {
		if(errno==ENOENT) {
			...
			for(i=0;i<liczba_elementow;i++) 
				semctl(semid,i,SETVAL,arg);     /*ustawienie wartosci poczatkowej semaforow na stan opuszczony*/
